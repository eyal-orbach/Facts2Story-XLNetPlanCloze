...
from PlotFactsOnlyDataset import PlotFactsOnlyDataset
...
def get_full_seqs_and_masks(pos_out,inputs, tokenizer):
    tensors =[]
    for i, val in enumerate(inputs):
        ...
        if i < len(inputs)-1:
            ...
            part[-1] = tokenizer.sep_token_id
        tensors.append(part)
    tensors.append(torch.tensor([tokenizer.cls_token_id]).to(inputs.device).float())
