...
class PlotFactsOnlyDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, dir_path: str, block_size=512, is_train=False):
        ...
        cached_features_file = os.path.join(
            dir_path,modelstr + "_cached_factsonly_end_fix4_" + str(block_size) + "_" + tokenizer_class
        )
        ...
        if os.path.exists(cached_features_file) and not args.overwrite_cache:
            ...
            for filename in os.listdir(dir_path):
                try:
                    ...
                    for fact in topfacts_sorted_by_positon:
                        fact_id +=1
                        encoded_fact = tokenizer.encode(fact.text, add_special_tokens=False)
                        facts_encoded += encoded_fact
                        facts_mapping += ([fact_id] * len(encoded_fact))
                        # for tok in tokenized_fact:
                        #     encoded_tok = tokenizer.convert_tokens_to_ids(tok, add_special_tokens=False)
                        #     facts_encoded += encoded_tok
                        #     facts_mapping += ([fact_id] * len(encoded_tok))

                    facts_encoded+= [tokenizer.eos_token_id]
                    facts_tensor=torch.tensor(facts_encoded).long()

                    facts_mapping+= [fact_id+1]
                    full_text_tensor = torch.tensor(ids_text)

                    self.raw_examples.append((full_text_tensor, facts_tensor,
                                              torch.tensor(facts_mapping, dtype=torch.long), genre))
                    good_docs +=1
                except:
                    bad_docs +=1
            ...
            for example in self.raw_examples:
                ...
                self.examples.append((example[0], example[1], example[2], torch.tensor(genre_key, dtype=torch.long)))
...
if __name__ == '__main__':
    ...
    path = "/home/nlp/eyalo/data/movie_plots/salie_plots_tokens_xlnet_details/selected"
    tokenizer = XLNetTokenizer.from_pretrained("xlnet-base-cased", cache_dir="/tmp/cache")
    mds = PlotFactsOnlyDataset(tokenizer, args, path)
