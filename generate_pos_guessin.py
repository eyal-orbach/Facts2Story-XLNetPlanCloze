...
from MaskedPlotFactEmbeddingDataset import MaskedPlotFactEmbeddingDataset
from MaskedPlotGenreDataset import MaskedPlotGenreDataset, GENRES_LIST
from custom_model.CXLNetConfig import PosNETConfig
...
from utils.TokenizationMapper import TokenizationMapper
...
RESULTS_OUT_PATH = "/home/nlp/eyalo/tmp/pycharm_project_711/customxlnet/examples/generation_e2e/xlnetlogpos_blind/"
...
MODEL_CLASSES = {
    'posnet': (PosNetHeadModel, XLNetTokenizer),
...
def printlogits(printinfo, text_untill):
    print("------------")
    print(text_untill)
    print("--")
    for k, v in sorted(printinfo.items(), key=lambda item: -item[1]['g']):
        print(f"{k}: xlnet: {v['x']}  gpt2: {v['g']}")



def sample_sequence(model, context, perm_masks, padding_masks, target_mappings, temperature=1, top_k=0, top_p=0.0,
                    device='cpu', genre_idxs=None, tokenizer=None, helper_model=None, helper_tokenizer=None):
    ...
    with torch.no_grad():
        for perm_list_idx in range(perm_masks.size(-1)):
            ...
            inputs = {'input_ids': input_ids, 'perm_mask': curr_perm_mask, 'target_mapping': curr_target_mapping, 'genre_idxs':genre_idxs}
            ...
            for i in range(outputs[0].size(0)):
                if target_mappings[i,perm_list_idx,perm_list_idx] > 0:
                    ...
                    ##########debugging code###########
                    filtered_logits = top_k_top_p_filtering(distrbution, top_k=top_k, top_p=top_p)
...
def get_pos_model(args):
    config = PosNETConfig.from_pretrained(args.pos_model_name_or_path, cache_dir=args.cache_dir,
                                          n_genres=args.ngenres, n_facts=args.nfacts)
    posmodel = PosNetHeadModel.from_pretrained(
        args.pos_model_name_or_path,
        from_tf=bool(".ckpt" in args.pos_model_name_or_path),
        config=config,
        cache_dir=args.cache_dir,
    )
    posmodel.eval()
    return posmodel.to(args.device)


def get_full_seqs_and_masks(pos_out,inputs):
    tensors =[]
    for i, val in enumerate(inputs):
        ntokens=pos_out[i]
        part = torch.zeros(ntokens).to(inputs.device)
        part[-1] = val
        tensors.append(part)
    token_seq = torch.cat(tensors,dim=-1).long()
    mask = torch.where(token_seq==0, torch.zeros(token_seq.size()).to(inputs.device),
                       torch.ones(token_seq.size()).to(inputs.device))
...
def main():
    ...
    def collate(examples: List[torch.Tensor]):
        ...
        token_ids, fact_embedding_ids = zip(*[get_inputs(seq, mask) for seq, mask, genre in examples])
        ...
        pad_labels = pad_sequence(labels, batch_first=True, padding_value=tokenizer.pad_token_id)
        pad_originals = pad_sequence(seqs, batch_first=True, padding_value=tokenizer.pad_token_id)
        torch.stack(genres)
        return list(zip(pad_seqs, pad_factsembeds, pad_labels, genres, pad_originals))

    test_dataset=MaskedPlotFactEmbeddingDataset(tokenizer, args, args.test_file_path, block_size=512)
    ...
    for batch in test_dataloader:
        try:
            batch_counter+=1
            with torch.no_grad():
                seqs, factsembs, labels, genres, origials = zip(*batch)
                ...
                padding_masks = torch.where(tlabels == tokenizer.pad_token_id,
                                            torch.ones_like(tlabels), torch.zeros_like(tlabels)).to(args.device)

                pos_outputs = pos_model(tseqs, fact_embeds=tfact_embeds, genre_idxs=tgenres, input_mask=padding_masks,labels=tlabels)
                seqs, masks = get_full_seqs_and_masks(pos_outputs[1][0].squeeze(-1).long(), tseqs[0])
                ...
                from MaskedPlotFactEmbeddingDataset import GENRES_LIST
                ...
                print(text)
                original_text = tokenizer.decode(origials[0])[len(PADDING_TEXT):]
